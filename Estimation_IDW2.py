...
def generate_estimation_report(estimated_blocks, composites_data, idw_params, search_params, block_sizes, 
                              tonnage_data=None, plot_info=None, density=2.7, project_name="Projet Minier"):
    ...
    # Histogramme des composites
    fig_comp, ax_comp = plt.subplots(figsize=(6, 4))
    n_bins = int(1 + 3.322 * math.log10(len(composite_values)))
    ...
    if estimated_blocks:
        ...
        block_values = [block['value'] for block in estimated_blocks]
        ...
        # Histogramme des blocs
        fig_block, ax_block = plt.subplots(figsize=(6, 4))
        n_bins = int(1 + 3.322 * math.log10(len(block_values)))
        ...
        summary_data = [
            ["Paramètre", "Valeur"],
            ["Nombre de blocs", str(len(estimated_blocks))],
    ...
    if tonnage_data and plot_info:
        ...
        if plot_info['method'] != 'between':
            ...
            metals_subset = tonnage_data['metals'][::3]
            ...
            elements.append(Paragraph(f"Méthode de coupure: Entre {plot_info['min_grade']:.2f} et {plot_info['max_grade']:.2f}", normal_style))
            
            between_data = [
                ["Paramètre", "Valeur"],
                ["Tonnage (t)", f"{tonnage_data['tonnages'][0]:,.0f}"],
                ["Teneur moyenne", f"{tonnage_data['grades'][0]:.3f}"],
                ["Métal contenu", f"{tonnage_data['metals'][0]:,.0f}"]
            ]
    ...
    if 'comp_hist_path' in locals():
        os.unlink(comp_hist_path)
    if 'block_hist_path' in locals():
        os.unlink(block_hist_path)
    if 'tonnage_graph_path' in locals():
        os.unlink(tonnage_graph_path)
            ...
            for y in y_range:
                for z in z_range:
                    ...
                    if block_count % 100 == 0:
                        progress_bar.progress(block_count / total_blocks)
        
        progress_bar.progress(1.0)
    
    return blocks, {'min': min_bounds, 'max': max_bounds}

def estimate_block_model(empty_blocks, composites, idw_params, search_params):
    estimated_blocks = []
...
def plot_3d_model(blocks, composites, envelope_data=None):
    fig = go.Figure()
    ...
    if envelope_data:
        # Créer une représentation wireframe à partir des faces du maillage
...
if uploaded_file:
    ...
    with tabs[0]:  # Modèle 3D
        ...
        with col1:
            if create_model_button:
                ...
                st.session_state.block_sizes = block_sizes
                ...
                if st.button("Exporter modèle de blocs (CSV)"):
                    ...
                    export_df['DENSITY'] = density
                    export_df['VOLUME'] = export_df['SIZE_X'] * export_df['SIZE_Y'] * export_df['SIZE_Z']
                    export_df['TONNAGE'] = export_df['VOLUME'] * density
        ...
        if "estimated_blocks" in st.session_state:
            ...
            if st.button("Générer le rapport PDF", type="primary"):
                with st.spinner('Génération du rapport en cours...'):
                    ...
                    idw_params = st.session_state.idw_params
                    search_params = st.session_state.search_params
                    block_sizes = st.session_state.block_sizes
    ...
    3. **Importez une enveloppe DXF** (facultatif) pour limiter l'estimation à une forme géologique spécifique
    4. **Définissez les paramètres IDW** comme la puissance et l'anisotropie
    5. **Créez le modèle de blocs** en définissant la taille des blocs
    6. **Estimez par IDW** pour obtenir les teneurs des blocs
    7. **Analysez les résultats** dans les onglets Statistiques et Tonnage-Teneur
    8. **Générez un rapport PDF** complet de l'estimation
    9. **Exportez le modèle de blocs** en format CSV pour une utilisation dans d'autres logiciels
    ...
    ### Format du fichier DXF
    
    Le fichier DXF peut contenir des entités 3D comme:
    - Polylignes 3D fermées
    - Faces 3D (3DFACE)
    - Maillages (MESH)
    
    Ces entités définissent l'enveloppe géologique qui peut être utilisée pour contraindre le modèle de blocs.
    
    ### À propos de l'estimation par inverse des distances
    
    L'estimation par inverse des distances (IDW) est une méthode d'interpolation qui attribue plus de poids aux échantillons proches qu'aux échantillons éloignés. Le poids diminue en fonction de la distance selon une fonction puissance:
    
    $w(d) = \\frac{1}{d^p}$
    
    - $w(d)$ est le poids attribué à un échantillon à distance $d$
    - $p$ est la puissance (généralement entre 1 et 5)
    
    Cette application permet d'effectuer une estimation par IDW et d'analyser les résultats de manière interactive.
    """)
